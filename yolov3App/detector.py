""""
Credits: Leo van der Meulen(https://github.com/lmeulen/PeopleCounter),
Aravind Pai(https://www.analyticsvidhya.com/blog/2020/05/social-distancing-detection-tool-deep-learning/)
...
def change_2_red(img,person,p1,p2):
    ...
    for i in risky:
        ...
        _ = cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 2)  # red color
    return img
...
def update_frame(image, people_indxs, class_ids, detected_boxes, conf_levels, colors, labels,
                 show_boxes, blur, box_all_objects, crowd_threshold):
    """
    ...
    if len(people_indxs) >= 1:
        ...
        for i in people_indxs.flatten():
            ...
            if classIDs[i] == 0:
                ...
                # find the midpoint of every person as a reference point for distance calculation
                midpoints.append(mid_point(x, y, w, h))
                people.append(i)
        ...
        dist = compute_distance(midpoints, count_people)
        # print('raw dist')
        ...
        #     print()
        thresh = 90
        p1, p2, d = find_closest(dist, count_people, thresh)

        # change color of bounding box of risky people
        risky = np.unique(p1 + p2)
        # df = pd.DataFrame({"p1": p1, "p2": p2, "dist": d})
        # print('distance ', df)
        for i in risky:
            x1, y1, w, h = detected_boxes[people[i]]
            cv2.rectangle(image, (x1, y1), (x1 + w, y1 + h), (0, 0, 255), 2)  # red color

    # write number of people in bottom corner
    text = "Persons: {}".format(count_people)
    cv2.putText(image, text, (10, image.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    if count_people > crowd_threshold:
        warningText = 'WARNING! There is a high chance of Corona virus spread.'
        cv2.putText(image, warningText, (10, image.shape[0] - 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        warningText = 'This place is too crowded. Stay safe. Follow social distancing.'
        cv2.putText(image, warningText, (10, image.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        frame, npeople = update_frame(frame, idxs, classIDs, boxes, confidences, COLORS, LABELS, showpeopleboxes,
                                      blurpeople, showallboxes, int(config['OUTPUT']['CrowdThreshold']))
